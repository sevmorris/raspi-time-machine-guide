...
- **Share:** `TimeMachine` → `/mnt/timemachine` (Time Machine enabled)
...
- **Filesystem:** `ext4` mounted at `/mnt/timemachine`
...
### 4.3 Avahi Service File (Optional)

While Samba's `mdns name = mdns` handles basic mDNS advertisement, you can create a custom Avahi service file for more explicit Time Machine discovery:
...
Save as `/etc/avahi/services/timemachine.service`, then restart Avahi:

```bash
sudo systemctl restart avahi-daemon
```

> **Note:** If you see "No service file found in /etc/avahi/services" in the Avahi logs, that's normal—Samba's built-in mDNS is sufficient for most setups.
...
==================================================================================================
TimeMachine
                              SERVER_NAME                   <pi-hostname>
                              USER_ID                       501
                              SMB_NEGOTIATE                 SMB 3.0.2
...
> **Note:** If the output is empty, ensure you're connected to the share first (Finder → Go → Connect to Server).
...
```
fruit_get_bandsize: Didn't find band-size key in [...sparsebundle/Info.plist]
